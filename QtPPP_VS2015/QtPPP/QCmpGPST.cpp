...
bool QCmpGPST::readRepFile(QString m_erpFileName)
{
	...
	{//首先判断用户是否传入erp路径
		readerp(m_erpFileName.toLatin1().data(),&m_erpData);
	}
	else if (!ErpfileName.isEmpty())
	{//若没有传入则读取当前目录erp文件
		readerp(ErpfileName.toLatin1().data(),&m_erpData);
	}
	else
	{
		isuseErp = false;
...
double QCmpGPST::YMD2GPSTime(int Year,int Month,int Day,int HoursInt,int Minutes,int Seconds,int *WeekN)//,int *GPSTimeArray
{
	double Hours = HoursInt + ((Minutes * 60) + Seconds)/3600.0;
	//Get JD
	double JD = 0.0;
...
void QCmpGPST::XYZ2SAE(double X,double Y,double Z,double *m_pSAZ,double *PX)
{
	...
	XYZ2BLH(PX[0],PX[1],PX[2],PX_BLH,elipsePara);//计算L出现Bug
	double B = PX_BLH[0];
	double L = PX_BLH[1];
	...
	if (m_pSAZ[0] == 0)
		return;
	m_pSAZ[2] = qAcos(qAbs(zz/m_pSAZ[0]));//Z(0,pi/2)出现bug
}
...
//XYZ 接收机近似坐标 m_SAZ（弧度）返回的计算结果 PX测站坐标
...
double QCmpGPST::getLeapSecond(int Year,int Month,int Day,int Hours/* =0 */,int Minutes/* =0 */,int Seconds/* =0 */)
{
...
//double ep = {1970,1,1,0,0,0}转换为gtime_t;
...
void QCmpGPST::sunmoonpos_eci(gtime_t tut, double *rsun, double *rmoon)
{
	...
	if (rmoon) {
		...
		rmoon[1]=rm*cose*cosp*sinl - rm*sine*sinp;//rmoon[1]=rm*(cose*cosp*sinl - sine*sinp); release 模式（最快速度）下被优化 无法计算
		rmoon[2]=rm*sine*cosp*sinl + rm*cose*sinp;//rmoon[2]=rm*(sine*cosp*sinl + cose*sinp);release 模式（最快速度）下被优化 无法计算
...
//	double JD = computeJD(2010,4,10,0,0,30);
	//原点为1970.1.1时刻的GPST转换为原点1970.1.1时刻UTC时间
