...
QTropDelay::QTropDelay(QString GrdFileName,QString ProjectionFun,QString Model)
{
	initVar();
	if (Model.contains("G"))
	{
		isGPT2 = true;
		openGrdFile(GrdFileName);
		getAllData();
	}
...
{
	m_PI = 3.1415926535897932385;//圆周率 弧度
	m_GrdFileName = "gpt2_5.grd";//路径+文件名
	tempLine = "";
	isReadAllData = false;
...
//计算Neil投影的干量(md)和湿量(mw)投影函数
//在中纬度比较适合，高纬度引起高程方向的偏差
void QTropDelay::getNeilParm(double E,double H,double Lat,int TDay,double &md,double &mw)
{
	...
	double ad = 0,bd = 0,cd = 0;//干分量的abc
	double aw = 0,bw = 0,cw = 0;//湿分量的abc
	double T0 = 28;
	//查找位置
	int flag = 0;
	for(int i = 0;i < 5;i++)
...
void QTropDelay::getVMF1Parm(double ah,double aw,double E,double Lat,double H,int TDay,double &md,double &mw)
{
	...
	if(Lat < 0)
	{
		double c0 = 0.062,c11 = 0.001,c10 = 0.006,ph = m_PI;//PI
...
{//E:卫星高度角
	...
	m_PTe.T -= 273.15;//转为华摄氏度
	return(m_PTe);
}

//GPT2估计+简化的Saastamoinen模型
...
//采用GPT2+Hopfield（天顶干湿延迟）+Neill+VMF1 投影函数
double QTropDelay::getGPT2HopfieldDelay(double MJD,int TDay,double E,double *pBLH,double *mf)
{
	if (!isReadAllData)
	{
		isGPT2 = true;
	...
	double ZHD = 0,ZWD = 0;//天顶干湿分量
	double mD = 0,mW = 0;//投影函数
	...
	if (mf) *mf = mW;//此处只返回湿延迟投影
	return (ZHD*mD);//只返回干延迟!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	//return (ZHD*mD+ZWD*mW);//加入投影函数总延迟
}

//采用GPT2+Sassta（天顶干湿延迟）+Neill+VMF1 投影函数
double QTropDelay::getGPT2SasstaMDelay(double MJD,int TDay,double E,double *pBLH,double *mf)
{
	if (!isReadAllData)
	{
		isGPT2 = true;
	...
	if (mf) *mf = mW;//此处只返回湿延迟投影
	return (ZHD*mD);//只返回干延迟!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	//return (ZHD*mD+ZWD*mW);//加入投影函数总 延迟
}

//GPTdata存储了 气压（hPa） 温度（C） 椭球高差值（Geoid undulation）（m）
...
//GPT2计算气温、气压、湿度递减速率、比湿、VMF1的ah、aw
/*
...
void QTropDelay::trop_gpt(double dmjd,double dlat,double dlon,double dhgt,double *pres,double *temp,double *undu) 
{
	...
	for (i = 0; i <= n; i++) {
		...
		for (j = 0; j <= im; j++) {
			...
			p[i][j] = 1.0/ipow(2,i)*SQRT(qPow(1.0-sinlat*sinlat,j))*sum1;
		}
	}
...
//计算 Global Mapping Functions (GMF)
/*****************************************************************************
 * Name        : trop_map_gmf
 * 
...
//UNN3M模型
#ifdef EIGEN_CORE_H
//pBLH:[rad.rad,m];年积日：Tday（int）； 卫星高度角E：（rad）
//resulat =[总延迟,天顶干延迟,干延迟投影函数,天顶湿延迟,湿延迟投影函数];
double QTropDelay::getUNB3mDelay(double *pBLH, double TDay, double E,double *mf)
{
	Vector3d BLH;
	BLH[0] = pBLH[0];BLH[1] = pBLH[1];BLH[2] = pBLH[2];
	VectorXd resulat = UNB3M(BLH,TDay,E);
	if(mf) *mf = resulat[4];//湿延迟函数
	return resulat[1]*resulat[2];//只返回干延迟！！！！！！！
	//return resulat[0];//返回总延迟
}
...
VectorXd =[RTROP,HZD,HMF,WZD,WMF];[总延迟,天顶干延迟,干延迟投影函数,天顶湿延迟,湿延迟投影函数];
*/
