...
readSP3.setSatlitSys("GC");//其中GPS,GLONASS,BDS,Galieo分别使用：字母G,R,C,E
readSP3.getAllData();

(3)然后调用getPrcisePoint(int PRN,double GPST,double *pXYZ,double *pdXYZ = NULL);
double pXYZ[3] ={0}，pdXYZ[3] = {0};//定义坐标和速度
readSP3.getPrcisePoint(32,514800,pXYZ,pdXYZ);//得到坐标和速度
或者readSP3.getPrcisePoint(32,514800,pXYZ);//得到坐标
...
#define  ACCELERATE  1 // 1：加速程序（getPrcisePoint函数时间由小到大） 0：不加速程序（时间乱序）
//坐标系统采用SP3自带坐标系统
...
public:
	...
	~QReadSP3(void);
	QVector< SP3Data > getAllData();//返回并读取所有数据（耗时）
